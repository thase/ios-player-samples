...
// Application Flags (Make sure you set thig correctly)
static Boolean const usePlaylist = false;
...
static NSString * const kViewControllerContentID = @"3666678807001";
...
- (void)viewDidLoad
{
    ...
    if(usePlaylist) {
        [self requestContentFromPlaybackServiceVideo];
    } else {
        [self requestContentFromPlaybackServicePlaylist];
    }
}

- (void)requestContentFromPlaybackServiceVideo
{
    [self.playbackService findVideoWithVideoID:kViewControllerContentID parameters:nil
...
- (void)requestContentFromPlaybackServicePlaylist
{

    [self.playbackService findPlaylistWithPlaylistID:kViewControllerContentID parameters:nil
